...
df2 <- t(df) %>% as.data.frame # better, but we still need a new column for state 
# and we need to get rid of the first row
...
# pivot.longer is taking a wide dataset and making it longer. 
#You want to pivot longer when there's a single variable taking up multiple columns
